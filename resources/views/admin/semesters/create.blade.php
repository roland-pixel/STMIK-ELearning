...
@section('title', 'Tambah Semester - Maroon Sneat Style')
@section('page_title', 'Tambah Semester')
@section('page_desc', 'Buat semester baru')
...
@section('content')
    <div class="max-w-3xl mx-auto space-y-4">
        ...
        <div class="rounded-2xl border border-slate-200 bg-white shadow-soft">
            <div class="p-6 sm:p-7">
                <div class="flex items-start justify-between gap-4">
                    <div>
                        <h2 class="text-lg font-semibold text-ink-900">Tambah Semester</h2>
                        <p class="text-sm text-ink-500 mt-1">Isi data semester sebelum menyimpan.</p>
                    </div>
                ...
                <form method="POST" action="{{ route('admin.semesters.store') }}" class="mt-6 space-y-5">
                    @csrf

                    {{-- Nama Semester --}}
                    <div>
                        <label class="block text-sm font-medium text-ink-700">Nama Semester</label>
                        <input type="text" name="nama_semester" value="{{ old('nama_semester') }}"
                    ...
                    <div>
                        ...
                        <select name="status_aktif"
                            class="mt-1 w-full rounded-xl border border-slate-300 bg-white px-3 py-2 text-sm
                                   focus:outline-none focus:ring-4 focus:ring-maroon-600/10 focus:border-maroon-700 transition"
                            required>
                            <option value="inactive" {{ old('status_aktif', 'inactive') === 'inactive' ? 'selected' : '' }}>
                                Inactive
                            </option>
                            <option value="active" {{ old('status_aktif') === 'active' ? 'selected' : '' }}>
                                Active
                            </option>
                        </select>
                        @error('status_aktif')
        ...
        <p class="text-xs text-ink-400 text-center">
            Pastikan rentang tanggal sesuai kalender akademik.
        </p>
    </div>
@endsection
