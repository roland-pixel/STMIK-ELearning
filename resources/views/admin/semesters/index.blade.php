...
@section('title', 'Kelola Semester - Maroon Sneat Style')
@section('page_title', 'Kelola Semester')
@section('page_desc', 'Atur periode semester aktif dan jadwalnya')
    ...
    <div class="space-y-4 max-w-full">
        ...
        <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <form method="GET" class="flex gap-2 w-full sm:w-auto">
                <input type="text" name="q" value="{{ $q }}" placeholder="Cari nama/status..."
        ...
        <div class="rounded-2xl border border-slate-200 bg-white shadow-soft">
            ...
            <div class="max-w-full min-w-0 overflow-x-auto">
                <table class="min-w-[780px] w-full divide-y divide-slate-200">
                    ...
                    <tbody class="divide-y divide-slate-200">
                        @forelse($semesters as $item)
                            <tr class="hover:bg-maroon-50/40 transition-colors">
                                ...
                                <td class="px-4 py-3">
                                    @if ($item->status_aktif === 'active')
                                        <span
                                            class="inline-flex items-center rounded-full bg-emerald-50 px-2.5 py-1 text-xs font-semibold text-emerald-700 ring-1 ring-emerald-200 whitespace-nowrap">
                                            Active
                                        </span>
                                    @else
                                        <span
                                            class="inline-flex items-center rounded-full bg-slate-100 px-2.5 py-1 text-xs font-semibold text-slate-700 ring-1 ring-slate-200 whitespace-nowrap">
                                            Inactive
                                        </span>
                                    @endif
                                </td>
                                ...
                                <td class="px-4 py-3 text-right">
                                    <div class="inline-flex items-center gap-2 whitespace-nowrap">
                                        {{-- Edit --}}
                                        <a href="{{ route('admin.semesters.edit', $item) }}"
                                        ...
                                        {{-- Hapus --}}
                                        <button type="button"
                                            onclick="document.getElementById('hapus-semester-{{ $item->id }}').showModal()"
                                    ...
                                    {{-- Modal konfirmasi --}}
                                    <dialog id="hapus-semester-{{ $item->id }}"
                                        ...
                                        <div
                                            ...
                                            <div class="flex items-start gap-3">
                                                ...
                                                <div class="flex-1">
                                                    ...
                                                    <p class="mt-1 text-sm text-slate-600">
                                                        Kamu akan menghapus:
                                                        <span
                                                            class="font-medium text-slate-900">{{ $item->nama_semester }}</span>.
                                            ...
                                            <div class="mt-5 flex justify-end gap-2">
                                                <button type="button"
                                                    onclick="document.getElementById('hapus-semester-{{ $item->id }}').close()"
                                                    class="rounded-xl border border-slate-300 bg-white px-4 py-2 text-sm hover:bg-slate-50 transition">
                                                    Batal
                                                </button>

                                                <form method="POST"
                                                    action="{{ route('admin.semesters.destroy', $item) }}">
                                                    @csrf
                                                    @method('DELETE')
                                                    <button type="submit"
