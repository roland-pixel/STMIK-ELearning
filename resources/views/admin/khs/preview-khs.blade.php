...
@section('content')
    <div class="bg-white border border-slate-200 rounded-xl overflow-hidden">
        ...
        <div class="flex items-center justify-between px-6 py-4 border-b border-slate-100">
            <div>
                <h2 class="text-xl font-serif font-normal text-slate-900 tracking-tight">
                    Kartu Hasil Studi
                </h2>
            ...
            <div class="flex gap-2">
                <form method="POST" action="{{ route('admin.khs.cetak') }}" target="_blank">
                    @csrf
                    <input type="hidden" name="mahasiswa_id" value="{{ $mahasiswa->id }}">
                    <input type="hidden" name="semester_id" value="{{ $semester->id }}">
                    <button type="submit"
                        class="inline-flex items-center gap-1.5 text-sm font-medium px-4 py-2 rounded-lg bg-maroon-700 text-white hover:bg-maroon-800 transition-colors">
                ...
                <a href="{{ route('admin.khs.index') }}"
                    class="inline-flex items-center gap-1.5 text-sm font-medium px-4 py-2 rounded-lg border border-slate-200 text-slate-600 hover:bg-slate-50 transition-colors">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div class="p-6 space-y-5">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 border border-slate-200 rounded-lg overflow-hidden">
                <div class="p-5">
                    ...
                    <p class="text-[15px] font-medium text-slate-900 mb-2.5">
                        {{ config('app.name', 'Sistem Akademik Kampus') }}</p>
                    <div class="space-y-1.5 text-sm">
                        <div class="flex gap-2 text-slate-500">
                            <span class="w-24 shrink-0">NIM</span>
                            <span class="font-medium text-slate-800">{{ $mahasiswa->nim }}</span>
                        </div>
                        <div class="flex gap-2 text-slate-500">
                            <span class="w-24 shrink-0">Nama</span>
                            <span class="font-medium text-slate-800">{{ $mahasiswa->user->nama_lengkap }}</span>
                        </div>
                        <div class="flex gap-2 text-slate-500">
                            <span class="w-24 shrink-0">Program Studi</span>
                            <span class="font-medium text-slate-800">{{ $mahasiswa->jurusan->nama_jurusan ?? '-' }}</span>
                        </div>
                    </div>
                </div>

                <div class="p-5 border-t md:border-t-0 md:border-l border-slate-200">
                    <p class="text-[10px] font-semibold uppercase tracking-widest text-maroon-700 mb-3">Detail Akademik</p>
                    <div class="space-y-1.5 text-sm">
                        <div class="flex gap-2 text-slate-500">
                            <span class="w-24 shrink-0">Semester</span>
                            <span class="font-medium text-slate-800">{{ $semester->nama_semester }}</span>
                        </div>
                        <div class="flex gap-2 text-slate-500 items-center">
                            <span class="w-24 shrink-0">Jumlah MK</span>
                            <span
                                class="text-xs font-semibold bg-emerald-50 text-emerald-700 border border-emerald-200 px-2 py-0.5 rounded-full">
                                {{ count($khsData) }} Mata Kuliah
                            </span>
                        </div>
                        <div class="flex gap-2 text-slate-500 pt-1">
                            <span class="w-24 shrink-0">Tgl. Cetak</span>
                            <span
                ...
                <table class="w-full text-sm border-collapse">
                    <thead class="bg-slate-50 border-b border-slate-200">
                        <tr>
                            ...
                            <th
                                ...
                                Kode MK</th>
                            <th
                                class="px-3 py-3 text-left text-[10px] font-semibold uppercase tracking-widest text-slate-500">
                                Mata Kuliah</th>
                            <th
                    ...
                    <tbody class="divide-y divide-slate-100 bg-white text-slate-700">
                        @forelse($khsData as $index => $item)
                            <tr class="hover:bg-slate-50 transition-colors">
                                ...
                                <td class="px-3 py-3 text-left text-xs font-semibold text-maroon-700 uppercase">
                                    {{ $item->kode_tampil }}
                                </td>
                                <td class="px-3 py-3 text-left">
                                    <span class="font-medium text-slate-800">{{ $item->nama_mk }}</span>
                                    @if ($item->jenis_mk === 'Spesial')
                                        <span class="block text-[11px] text-slate-400 italic mt-0.5">Mata kuliah bimbingan
                                            terpadu</span>
                                    @endif
                                </td>
                                ...
                                @if ($item->jenis_mk === 'Spesial')
                                    <td colspan="3" class="px-3 py-3 text-center text-xs text-slate-400 italic">Nilai
                                        Bimbingan</td>
                                @else
                                ...
                                <td
                                    class="px-3 py-3 text-center font-semibold tabular-nums
                            {{ $item->nilai_akhir_angka >= 80 ? 'text-emerald-600' : ($item->nilai_akhir_angka >= 60 ? 'text-amber-600' : 'text-rose-600') }}">
                                ...
                                <td class="px-3 py-3 text-center font-medium">{{ $item->nilai_huruf ?? '-' }}</td>
                                <td class="px-3 py-3 text-center font-medium tabular-nums">
                    ...
                    <tfoot class="bg-maroon-700 text-white">
                        <tr>
                            ...
                            <td class="px-3 py-3.5 text-center text-lg font-semibold tabular-nums">{{ $totalSKS }}
                            </td>
                            <td colspan="3"
                                class="px-3 py-3.5 text-right text-[10px] font-semibold uppercase tracking-widest opacity-75">
            ...
            <div class="flex justify-between items-end pt-2">
                ...
                <div class="text-right">
                    <p class="text-[10px] font-semibold uppercase tracking-widest text-slate-400 mb-8">
                        Ketua Program Studi / Admin Akademik
                    </p>
