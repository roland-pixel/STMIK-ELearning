...
@section('title', 'Edit Dosen - Maroon Sneat Style')
@section('page_title', 'Edit Dosen')
@section('page_desc', 'Ubah data akun user + data dosen')
...
@section('content')
    <div class="max-w-3xl mx-auto space-y-4">
        ...
        <div class="rounded-2xl border border-slate-200 bg-white shadow-soft">
            <div class="p-6 sm:p-7">
                <div class="flex items-start justify-between gap-4">
                    <div>
                        <h2 class="text-lg font-semibold text-ink-900">Edit Dosen</h2>
                        <p class="text-sm text-ink-500 mt-1">Perbarui data user dan dosen sebelum menyimpan.</p>
                    </div>
                ...
                <form method="POST" action="{{ route('admin.dosens.update', $dosen) }}" class="mt-6 space-y-6">
                    @csrf
                    @method('PUT')
                        ...
                        <div class="mt-4 space-y-4">
                            <div>
                                ...
                                <input type="text" name="nama_lengkap"
                                    value="{{ old('nama_lengkap', $dosen->user->nama_lengkap) }}"
                ...
                focus:outline-none focus:ring-4 focus:ring-maroon-600/10 focus:border-maroon-700 transition"
                                ...
                                @foreach ($jurusans as $j)
                                    <option value="{{ $j->nama_jurusan }}" {{-- Logika: Utamakan input lama (jika error), kalau tidak ada pakai data dari DB --}}
                                        {{ old('homebase', $dosen->homebase) == $j->nama_jurusan ? 'selected' : '' }}>
        ...
        <p class="text-xs text-ink-400 text-center">
            Perubahan email/password akan mempengaruhi akses login dosen.
        </p>
    </div>
@endsection
