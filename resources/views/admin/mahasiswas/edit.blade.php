...
@section('title', 'Edit Mahasiswa - Maroon Sneat Style')
@section('page_title', 'Edit Mahasiswa')
@section('page_desc', 'Ubah data akun user + data mahasiswa')
        ...
        <div class="rounded-2xl border border-slate-200 bg-white shadow-soft">
            <div class="p-6 sm:p-7">
                <div class="flex items-start justify-between gap-4">
                    <div>
                        <h2 class="text-lg font-semibold text-ink-900">Edit Mahasiswa</h2>
                        <p class="text-sm text-ink-500 mt-1">Perbarui data user dan mahasiswa sebelum menyimpan.</p>
                    </div>
                ...
                <form method="POST" action="{{ route('admin.mahasiswas.update', $mahasiswa) }}" class="mt-6 space-y-6">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="rounded-2xl border border-slate-200/70 bg-slate-50/40 p-4 sm:p-5">
                        ...
                        <div class="mt-4 space-y-4">
                            ...
                            <div>
                                <label class="block text-sm font-medium text-ink-700">Password Baru (opsional)</label>
                                ...
                                @error('password')
                                    <p class="mt-1 text-sm text-rose-700">{{ $message }}</p>
                                @enderror
                                <p class="mt-1 text-xs text-ink-500">Kosongkan jika tidak ingin mengubah password.</p>
                            </div>
                        </div>
                    </div>
                            ...
                            <div>
                                ...
                                <select name="jurusan_id"
                                    ...
                                    @foreach ($jurusans as $j)
                                        <option value="{{ $j->id }}"
                                            {{ (string) old('jurusan_id', $mahasiswa->jurusan_id) === (string) $j->id ? 'selected' : '' }}>
                            ...
                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <select name="jenis_program"
                                        ...
                                        <option value="reguler"
                                            {{ old('jenis_program', $mahasiswa->jenis_program) === 'reguler' ? 'selected' : '' }}>
                                            reguler</option>
                                        <option value="malam"
                                            {{ old('jenis_program', $mahasiswa->jenis_program) === 'malam' ? 'selected' : '' }}>
                                            malam</option>
                                        <option value="pegawai"
                                            {{ old('jenis_program', $mahasiswa->jenis_program) === 'pegawai' ? 'selected' : '' }}>
                                            pegawai</option>
                                    </select>
                                    @error('jenis_program')
                                        <p class="mt-1 text-sm text-rose-700">{{ $message }}</p>
                                ...
                                <div>
                                    ...
                                    <select name="status_masuk"
                                        ...
                                        required>
                                        <option value="normal"
                                            {{ old('status_masuk', $mahasiswa->status_masuk) === 'normal' ? 'selected' : '' }}>
                                            normal</option>
                                        <option value="transfer"
                                            {{ old('status_masuk', $mahasiswa->status_masuk) === 'transfer' ? 'selected' : '' }}>
                                            transfer</option>
                                    </select>
                                    @error('status_masuk')
                                        <p class="mt-1 text-sm text-rose-700">{{ $message }}</p>
