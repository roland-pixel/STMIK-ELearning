...
/**
 * Key stabil per pertanyaan:
 * - Existing pertanyaan: "q_<id>"
 * - Pertanyaan baru: UUID
 */
const makeClientKey = () => {
...
const form = useForm({
    ...
    tenggat_waktu: isoToDatetimeLocal(props.penilaian?.tenggat_waktu),

    pertanyaans: props.penilaian?.pertanyaans?.length
        ? props.penilaian.pertanyaans.map((q, idx) => ({
              id: q.id ?? null,
              client_key: q.id ? `q_${q.id}` : makeClientKey(), // ✅ stabil
              nomor_urut: q.nomor_urut ?? idx + 1,
              text_pertanyaan: q.text_pertanyaan ?? "",
              jenis_pertanyaan: q.jenis_pertanyaan ?? "essai",
              bobot_soal: Number(q.bobot_soal ?? 0),
              opsi_jawabans:
                  q.jenis_pertanyaan === "pilihan_ganda"
                      ? q.opsi_jawabans?.length
                          ? q.opsi_jawabans.map((o) => ({
                                id: o.id ?? null,
...
/**
 * ✅ imagesNew & previews keyed by client_key (bukan index)
 * imagesNew[client_key] = File[]
 * previews[client_key]  = string[]
 */
const imagesNew = reactive({});
const previews = reactive({});
const removeImageIds = ref([]); // image IDs yang mau dihapus (ke backend)
...
const removePertanyaan = (index) => {
    ...
    const q = form.pertanyaans[index];

    // tandai existing images untuk dihapus
    (q?.images || []).forEach((img) => {
        if (img?.id) removeImageIds.value.push(img.id);
    });

    // cleanup previews berdasarkan client_key
    const key = q?.client_key;
    if (key && previews[key])
        previews[key].forEach((u) => URL.revokeObjectURL(u));
    if (key) {
        delete imagesNew[key];
        delete previews[key];
    }
...
const removeOpsi = (q, idx) => {
    ...
    const removedWasTrue = !!q.opsi_jawabans[idx]?.is_benar;
    q.opsi_jawabans.splice(idx, 1);
    if (removedWasTrue && !q.opsi_jawabans.some((o) => o.is_benar)) {
...
/**
 * ✅ upload pick images berdasarkan q.client_key
 */
const onPickImages = (q, event) => {
    ...
    // revoke old previews
    if (previews[key]) previews[key].forEach((u) => URL.revokeObjectURL(u));

    imagesNew[key] = files;
    previews[key] = files.map((f) => URL.createObjectURL(f));

    // reset value agar pilih file yang sama lagi tetap trigger change
    event.target.value = "";
};

const removeNewImageAt = (q, imgIndex) => {
    const key = q.client_key;
    const files = imagesNew[key] || [];
    const urls = previews[key] || [];
    ...
    if (nextFiles.length) {
        imagesNew[key] = nextFiles;
        previews[key] = nextUrls;
    } else {
        delete imagesNew[key];
        delete previews[key];
    }
...
const removeExistingImage = (q, img) => {
    if (!img?.id) return;

    if (!removeImageIds.value.includes(img.id))
        removeImageIds.value.push(img.id);
...
const buildFormData = () => {
    ...
    // remove image ids
    (removeImageIds.value || []).forEach((id, idx) => {
        fd.append(`remove_image_ids[${idx}]`, String(id));
    });

    // pertanyaans
    form.pertanyaans.forEach((q, i) => {
        // ✅ kirim id kalau ada (biar update bukan create)
        if (q.id) fd.append(`pertanyaans[${i}][id]`, String(q.id));

        // ✅ kirim client_key wajib
        fd.append(`pertanyaans[${i}][client_key]`, String(q.client_key));
    ...
    // ✅ upload images baru: images[client_key][]
    Object.keys(imagesNew).forEach((key) => {
        (imagesNew[key] || []).forEach((file) =>
...
<template>
    <section class="min-h-screen bg-gray-50/60">
            ...
            <div class="mx-auto max-w-6xl px-6 py-5">
                <div class="flex items-start justify-between gap-4">
                    <div class="min-w-0">
                        ...
                        <h1
                            class="mt-1 text-xl font-bold text-gray-900 tracking-tight"
                        >
                            Edit Penilaian Online
                        </h1>
                        <p class="mt-1 text-sm text-gray-600">
                            Ubah judul, tenggat, dan pertanyaan.
                        </p>
                    </div>
        ...
        <div class="mx-auto max-w-6xl px-6 py-6 space-y-6">
                ...
                <div class="p-5">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                        ...
                        <div class="md:col-span-1">
                            ...
                            <input
                                v-model="form.tenggat_waktu"
                                type="datetime-local"
                                class="mt-2 w-full rounded-xl border-2 border-gray-200 bg-gray-50 px-4 py-3 text-sm text-gray-900 focus:border-blue-500 focus:bg-white focus:ring-4 focus:ring-blue-100"
                            />
                            <div
                                v-if="form.errors.tenggat_waktu"
                                class="mt-2 text-xs text-rose-600"
                            >
                        ...
                        <div class="md:col-span-1">
                            ...
                            >
                                ...
                                <ul
                                    class="mt-2 space-y-1 text-xs text-gray-600 list-disc pl-4"
                                >
                                    <li>
                                        Kamu bisa hapus gambar lama per
                                        pertanyaan.
                                    </li>
                                    <li>
                                        Gambar baru akan ditambahkan saat
                                        disimpan.
                                    </li>
                                    <li>
                                        Struktur pertanyaan akan di-replace
                                        sesuai form saat ini.
                                    </li>
                                </ul>
                            </div>
                        </div>
            ...
            <div class="rounded-2xl border border-gray-200 bg-white shadow-sm">
                ...
                <div class="p-5 space-y-5">
                    <div
                        v-for="(q, i) in form.pertanyaans"
                        :key="q.client_key"
                        class="rounded-2xl border-2 border-gray-200 bg-white"
                    >
                        <!-- header item -->
                        <div
                            class="px-4 py-3 border-b border-gray-100 flex items-center justify-between gap-3"
                        >
                            ...
                            <button
                                ...
                                :disabled="form.pertanyaans.length <= 1"
                                @click="removePertanyaan(i)"
                            >
                                Hapus
                            </button>
                        </div>
                        ...
                        <div class="p-4">
                            <div class="grid grid-cols-1 lg:grid-cols-3 gap-4">
                                ...
                                <div class="lg:col-span-1 space-y-4">
                                    <div>
                                        ...
                                        <select
                                            v-model="q.jenis_pertanyaan"
                                            class="mt-2 w-full rounded-xl border-2 border-gray-200 bg-gray-50 px-4 py-3 text-sm text-gray-900 focus:border-blue-500 focus:bg-white focus:ring-4 focus:ring-blue-100"
                                            @change="onJenisChange(q)"
                                        >
                                            <option
                                                v-for="opt in jenisPertanyaanOptions"
                                                :key="opt.value"
                                                :value="opt.value"
                                            >
                                                {{ opt.label }}
                                    ...
                                    >
                                        ...
                                        <div class="mt-1 text-xs text-gray-600">
                                            Bisa hapus gambar lama atau tambah
                                            gambar baru.
                                        </div>
                            ...
                            <!-- existing images -->
                            <div v-if="q.images?.length" class="mt-4">
                                <div
                                ...
                                <div class="mt-2 flex flex-wrap gap-3">
                                    ...
                                    >
                                        ...
                                        <button
                                            ...
                                            @click="removeExistingImage(q, img)"
                                        >
                                            Hapus
                                        </button>
                                    </div>
                            ...
                            <!-- new previews -->
                            <div
                                v-if="previews[q.client_key]?.length"
                                class="mt-4"
                            >
                                <div
                                    class="text-[11px] font-bold tracking-wide text-gray-700 uppercase"
                            ...
                            <!-- pilihan ganda -->
                            <div
                                v-if="q.jenis_pertanyaan === 'pilihan_ganda'"
                                class="mt-5"
                            >
                                <div
                            ...
                            <!-- upload file info -->
                            <div
                                v-if="q.jenis_pertanyaan === 'upload_file'"
        ...
        >
            <div
                class="mx-auto max-w-6xl px-6 py-3 flex items-center justify-between gap-3"
            >
