...
// ✅ goItem() - SAME LOGIC AS FORUM TAB
const goItem = (it) => {
    if (!it) return;

    console.log('TugasTab clicked:', it);  // DEBUG
...
// ✅ penilaianItems mapping - SAME AS FORUM TAB
...
const isEmpty = computed(() => {
    return (props.materis?.length ?? 0) === 0 && (props.penilaians?.length ?? 0) === 0;
});
...
const pickCreate = (type) => {
    ...
    if (type === "penilaian_online") {
        router.visit(route("dosen.kelas.penilaian.online.create", props.kelas.uuid));
    }
    if (type === "penilaian_manual") {
        router.visit(route("dosen.kelas.penilaian.manual.create", props.kelas.uuid));
    }
    closeCreate();
};
...
const materiFileUrl = (m) => {
    ...
    const url = safeRoute("kelas.materi.file.view", { kelas: props.kelas.uuid, materi: m.id }, `/storage/${m.file_path}`);
    return url;
};
...
const deleteMateri = (m) => {
    ...
    router.delete(route("dosen.kelas.materi.destroy", [props.kelas.uuid, m.id]), { preserveScroll: true });
};

// ================= Penilaian Actions (Menu) =================
const goEditPenilaianOnline = (p) => {
    openMenuId.value = null;
    const url = safeRoute("dosen.kelas.penilaian.online.edit", { kelas: props.kelas.uuid, penilaian: p.uuid }, "#");
...
const deletePenilaianOnline = (p) => {
    ...
    const url = safeRoute("dosen.kelas.penilaian.online.destroy", { kelas: props.kelas.uuid, penilaian: p.uuid }, null);
    if (!url) return;
    router.delete(url, { preserveScroll: true, onFinish: () => (openMenuId.value = null) });
};
</script>

<template>
    <!-- SAME TEMPLATE - TAPI UBAH ONCLICK ke goItem() -->
    <section class="w-full min-w-0">
                ...
                <div class="relative" data-create-wrap>
                    ...
                    <div v-if="openCreate"
                        ...
                        <button type="button" class="w-full flex items-center gap-3 px-4 py-3 text-sm hover:bg-gray-50"
                            ...
                            <span
                                class="grid h-9 w-9 place-items-center rounded-xl bg-gray-50 ring-1 ring-gray-200">📘</span>
                            <div>
                                <div class="font-medium">Materi</div>
                                <div class="text-[11px] text-gray-500">Upload file / link</div>
                            </div>
                        </button>
                        <button type="button" class="w-full flex items-center gap-3 px-4 py-3 text-sm hover:bg-gray-50"
                            ...
                            <div>
                                ...
                                <div class="text-[11px] text-gray-500">Buat soal essai / PG / upload</div>
                            </div>
                        </button>
                        <button type="button"
                            ...
                            <div>
                                ...
                                <div class="text-[11px] text-gray-500">Input nilai langsung</div>
                            </div>
                        </button>
                    </div>
                </div>
        ...
        <div class="mx-auto max-w-6xl px-6">
            ...
            <div v-if="isEmpty" class="py-20 flex flex-col items-center text-center">
                ...
                <div class="mt-6 text-sm font-semibold text-gray-800">Di sinilah Anda akan memberikan tugas</div>
                <div class="mt-2 max-w-md text-sm text-gray-500">
                    Anda dapat menambahkan tugas dan pekerjaan lain untuk kelas, lalu mengaturnya ke dalam topik.
                </div>
            </div>

            <!-- LIST -->
            <div v-else class="py-6 space-y-6">
                <!-- ✅ PENILAIAN LIST - NEW goItem() -->
                <div v-if="penilaians.length" class="space-y-3">
                    <div class="text-xs font-semibold text-gray-500 uppercase tracking-wide">Penilaian</div>

                    <div v-for="p in penilaians" :key="p.id" class="rounded-2xl border transition cursor-pointer"
                        :class="openPenilaianId === p.id ? 'border-blue-600 bg-gray-50' : 'border-gray-200 bg-white hover:bg-gray-50'"
                        @click="goItem(penilaianItems.find(item => item.id === `penilaian-${p.id}`))">

                        <!-- ROW (SAMA TAPI ONCLICK DI PARENT) -->
                        <div class="px-5 py-4 flex items-center justify-between gap-4">
                            <!-- KIRI -->
                            <div class="flex items-center gap-4 min-w-0">
                                <div class="shrink-0">
                                    <div class="w-10 h-10 rounded-full bg-gradient-to-br from-blue-500 to-blue-600 grid place-items-center text-white shadow-lg"
                                        title="Penilaian">
                                ...
                                <div class="min-w-0">
                                    <div class="flex items-center gap-2 min-w-0">
                                        <div class="text-sm font-semibold text-gray-900 truncate">{{ p.judul }}</div>
                                        <span
                                    ...
                                    <div class="mt-1 text-xs text-gray-500 truncate" v-if="p.instruksi">{{ p.instruksi
                                        }}</div>
                                </div>
                            </div>
                                ...
                                <div class="relative" data-row-menu @click.stop>
                                    ...
                                    <div v-if="openMenuId === `p-${p.id}`"
                                        ...
                                        <button type="button"
                                            ...
                                            @click="goEditPenilaianOnline(p)">Edit</button>
                                        <button type="button"
                                            class="w-full text-left px-4 py-3 text-sm hover:bg-gray-50 text-rose-700"
                                            @click="deletePenilaianOnline(p)">Hapus</button>
                                    </div>
                                </div>
                            </div>
                        </div>
